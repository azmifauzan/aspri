    ...
    <div className="p-4 md:p-6">
       <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6">
        <div className="flex items-center">
            <button onClick={() => setActiveItem('finance')} className="p-2 mr-2 text-zinc-500 hover:text-brand dark:hover:text-brand-dark rounded-full hover:bg-zinc-100 dark:hover:bg-zinc-700">
                <ArrowLeft size={20} />
            </button>
            <h1 className="text-2xl font-bold text-zinc-900 dark:text-white">Manajemen Kategori</h1>
        </div>
      ...
      <div className="bg-white dark:bg-zinc-800 rounded-xl shadow-sm">
        <div className="overflow-x-auto">
            {loading ? (
                <p className="p-6 text-center">Loading categories...</p>
            ) : error ? (
            ...
            <table className="w-full text-sm text-left text-zinc-500 dark:text-zinc-400">
                ...
                {categories.map((category) => (
                    <tr key={category.id} className="bg-white dark:bg-zinc-800 border-b dark:border-zinc-700 hover:bg-zinc-50 dark:hover:bg-zinc-700/50">
                        ...
                        <td className="px-6 py-4">{category.type === 'income' ? 'Pemasukan' : 'Pengeluaran'}</td>
